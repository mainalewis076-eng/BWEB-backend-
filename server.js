...
}catch(e){return r.status(500).json({error:"Failed to create order"})}
});
...
await sb.from("notifications").insert([{user_id:o.seller_id,type:"new_order",title:"New Order!",message:${o.buyer_name} ordered ${o.product_name}. You get $${o.seller_amount} after delivery.,order_id:id,read:false}]);
}}
r.send("OK");
}catch{r.send("Error")}
});
app.get("/payment-callback",async(q,r)=>{
const{order_id}=q.query;
const{data:o}=await sb.from("orders").select("payment_status").eq("id",order_id).single();
r.redirect(o?.payment_status==="paid"?${WEB}?success=1:${WEB}?failed=1);
});
...
const{data:o}=await sb.from("orders").select("*").eq("id",order_id).single();
if(!o)return r.status(404).json({error:"Not found"});
if(o.delivery_confirmed)return r.status(400).json({error:"Already confirmed"});
...
const orders=o||[];
r.json({success:true,orders,stats:{total_orders:orders.length,total_revenue:orders.filter(x=>x.delivery_confirmed).reduce((s,x)=>s+x.seller_amount,0)}});
});
...
app.listen(PORT,()=>console.log("BWEB running on port "+PORT));
